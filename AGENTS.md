...
- **Use `release-notes` when release note or changelog writing is needed.** Follow `.kilo/skills/release-notes/SKILL.md` whenever the user asks to write release notes, changelog entries, release summaries, or version notes from git history. Always ask one question first: which commit or date to start from. Then inspect git history for that range and build release notes only from verified commits.
